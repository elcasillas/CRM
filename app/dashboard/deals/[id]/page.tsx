...
export default function DealDetailPage() {
  ...
  const fetchDeal = useCallback(async () => {
    ...
    // Populate stored summary / inspection if present
    if ((data as unknown as DealData).ai_summary) {
      setSummary((data as unknown as DealData).ai_summary)
      setSummaryGeneratedAt((data as unknown as DealData).ai_summary_generated_at ?? null)
    }
    if ((data as unknown as DealData).inspection_result) {
      setInspection((data as unknown as DealData).inspection_result)
    }
  }, [id])
  ...
  useEffect(() => {
    async function init() {
      ...
      // Load email map for admin/SM
      if (role === 'admin' || role === 'sales_manager') {
        fetch('/api/admin/users').then(res => {
          if (res.ok) res.json().then((users: { id: string; email: string }[]) => {
  ...
  async function generateSummary() {
    ...
    try {
      ...
      if (res.ok) { setSummary(body.summary); setSummaryGeneratedAt(body.generatedAt ?? null) }
  ...
  async function runInspection() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.result) setInspection(data.result as InspectionResult)
      }
    } catch (_e) { /* silent */ }
  ...
  async function handleEmailOwner() {
    if (!deal) return
    const ownerEmail = emailMap.get(deal.deal_owner_id) ?? ''
    ...
    if (!summary) {
      ...
      try {
        ...
        if (res.ok) {
          const body = await res.json()
          if (body.summary) { setSummary(body.summary); setSummaryGeneratedAt(body.generatedAt ?? null) }
        }
      } catch (_e) { /* continue */ }
    }
    ...
    const fallbackSubject = `Deal Update: ${deal.deal_name}`
    const fallbackBody = `Hi ${deal.deal_owner?.full_name ?? 'there'},\n\nI wanted to follow up on "${deal.deal_name}" (${deal.deal_stages?.stage_name ?? 'unknown stage'}).\n\nCould you please provide a current status update and flag any blockers?\n\nThanks.`
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Right — AI + Inspection (admin/SM only) */}
        {canViewAI && (
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  {summaryGeneratedAt && (
                    <span className="text-xs text-gray-400">· {relativeTime(summaryGeneratedAt)}</span>
                  )}
                </div>
                <button
                    ...
                    if (block.startsWith('## ')) {
                      ...
                      return (
                        <div key={i} className={i > 0 ? 'mt-4' : ''}>
                          <p className="text-sm font-semibold text-gray-900 mb-1 leading-snug">{heading}</p>
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
              ...
              {inspection ? (
                ...
                <p className="text-xs text-gray-400">
                  {inspectionLoading ? 'Running inspection…' : 'Click Run Inspection to evaluate this deal.'}
                </p>
              )}
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {emailStatus === 'checking'    ? 'Checking…' :
                   emailStatus === 'summarizing' ? 'Summarizing…' :
                   emailStatus === 'inspecting'  ? 'Inspecting…' :
                   emailStatus === 'emailing'    ? 'Generating…' :
                ...
                {deal.deal_owner?.slack_member_id && (
                  <a
                    href={`slack://user?team=${slackTeamId}&id=${deal.deal_owner.slack_member_id}`}
