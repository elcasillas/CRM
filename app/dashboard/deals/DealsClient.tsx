                  ...
                  <tr key={deal.id} className="hover:bg-brand-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5 font-medium text-gray-900 max-w-[220px]">
                      <div className="flex items-center gap-1.5 min-w-0">
                        ...
                        <button onClick={() => openFeedback(deal)} title="View deal details" className="shrink-0 text-gray-400 hover:text-brand-600 transition-colors">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 bg-brand-700 rounded-t-xl shrink-0">
              <div className="flex items-center gap-3 min-w-0">
                <h3 className="font-semibold text-white truncate">{feedbackDeal.deal_name}</h3>
                {feedbackDeal.deal_stages && (
                  <span className={`shrink-0 text-xs font-medium px-2 py-0.5 rounded ${stageBadgeClass(feedbackDeal.deal_stages)}`}>{feedbackDeal.deal_stages.stage_name}</span>
                )}
                {feedbackDeal.health_score != null && (
                  <span className={`shrink-0 text-xs font-semibold px-2 py-0.5 rounded ${healthBadgeClass(feedbackDeal.health_score)}`}>{feedbackDeal.health_score}</span>
                )}
              </div>
              <button onClick={closeFeedback} className="text-white/70 hover:text-white text-lg leading-none ml-4 shrink-0">✕</button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                {feedbackDeal.accounts?.account_name && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Account</p><p className="text-sm text-gray-900 mt-0.5">{feedbackDeal.accounts.account_name}</p></div>
                )}
                <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Deal Owner</p><p className="text-sm text-gray-900 mt-0.5">{feedbackDeal.deal_owner?.full_name ?? '—'}</p></div>
                {feedbackDeal.solutions_engineer?.full_name && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Solutions Engineer</p><p className="text-sm text-gray-900 mt-0.5">{feedbackDeal.solutions_engineer.full_name}</p></div>
                )}
                {feedbackDeal.value_amount != null && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">ACV</p><p className="text-sm text-gray-900 mt-0.5">{formatCurrency(feedbackDeal.value_amount)}</p></div>
                )}
                {feedbackDeal.total_contract_value != null && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">TCV</p><p className="text-sm text-gray-900 mt-0.5">{formatCurrency(feedbackDeal.total_contract_value)}</p></div>
                )}
                {feedbackDeal.close_date && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Close Date</p><p className="text-sm text-gray-900 mt-0.5">{formatClose(feedbackDeal.close_date)}</p></div>
                )}
                {feedbackDeal.currency && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Currency</p><p className="text-sm text-gray-900 mt-0.5">{feedbackDeal.currency}</p></div>
                )}
                {feedbackDeal.contract_term_months != null && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Contract Term</p><p className="text-sm text-gray-900 mt-0.5">{feedbackDeal.contract_term_months} months</p></div>
                )}
                {feedbackDeal.region && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Region</p><p className="text-sm text-gray-900 mt-0.5">{feedbackDeal.region}</p></div>
                )}
                {feedbackDeal.deal_type && (
                  <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Type</p><p className="text-sm text-gray-900 mt-0.5">{feedbackDeal.deal_type}</p></div>
                )}
                {(() => { const ts = lastNoteDates.get(feedbackDeal.id); if (!ts) return null; const days = Math.floor((Date.now() - new Date(ts).getTime()) / 86400000); return <div><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Days Since Update</p><p className="text-sm text-gray-900 mt-0.5">{days}</p></div> })()}
                {feedbackDeal.deal_description && (
                  <div className="col-span-2"><p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Description</p><p className="text-sm text-gray-700 mt-0.5">{feedbackDeal.deal_description}</p></div>
                )}
              </div>
                ...
                <div className="border-t border-gray-100 pt-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      {feedbackSummaryGeneratedAt && <span className="text-xs text-gray-400">· {relativeTime(feedbackSummaryGeneratedAt)}</span>}
                    </div>
                    <button onClick={handleRegenerateSummary} disabled={loadingFeedbackSummary} className="text-xs text-brand-600 hover:text-brand-700 disabled:opacity-50 font-medium">
                ...
                <div className="border-t border-gray-100 pt-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      {inspection?.runAt && (
                        <span className="text-xs text-gray-400">{(() => { const m = Math.floor((Date.now() - new Date(inspection.runAt).getTime()) / 60000); return m < 2 ? 'just now' : m < 60 ? `${m}m ago` : `${Math.floor(m / 60)}h ago` })()}</span>
                      )}
                    </div>
                ...
                <div className="border-t border-gray-100 pt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {(() => {
                      ...
                      const slackMemberId = ownerProfile?.slack_member_id
              ...
              <div className="border-t border-gray-100 pt-4">
                ...
                {feedbackNote ? (
                  <div className="bg-gray-50 border border-gray-100 rounded-xl p-4">
                    <p className="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">{feedbackNote.note_text}</p>
                    <p className="text-xs text-gray-400 mt-2">{feedbackNote.author?.full_name ?? 'Unknown'} · {fmtTs(feedbackNote.created_at)}</p>
                  </div>
                ) : (
