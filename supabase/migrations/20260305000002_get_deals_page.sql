-- Returns the full deals-page dataset in one round-trip.
-- Combines the deals+joins query and the notes last-activity query that were
-- previously run as two separate queries from the server component.
CREATE OR REPLACE FUNCTION public.get_deals_page(
...
AS $$
  ...
  WHERE
    -- optional search across deal name and account name
    (  p_search IS NULL
