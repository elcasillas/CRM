-- =============================================================
-- Migration: Add region and deal_type columns to deals;
--            update get_deals_page RPC to return them.
-- =============================================================

-- 1. Add new columns to deals
ALTER TABLE public.deals
  ADD COLUMN IF NOT EXISTS region    text NULL
    CHECK (region IN ('North America', 'Europe/Asia/Pacific/Africa', 'Latin America/Caribbean')),
  ADD COLUMN IF NOT EXISTS deal_type text NULL
    CHECK (deal_type IN ('Migration', 'Organic One-Time', 'Organic Recurring', 'Pro Services'));

-- 2. Recreate get_deals_page to include region and deal_type.
--    Drops the existing function (signature from 20260306000008_rpc_active_only)
--    then recreates it with two new columns added to RETURNS TABLE and SELECT.
DROP FUNCTION IF EXISTS public.get_deals_page(text, uuid, uuid, boolean, boolean, int, boolean);
...
AS $$
  SELECT
    ...
    -- last note timestamp via lateral aggregation
    ln.last_note_at,
    COALESCE(
